...
###### create docker-registry secret for dockerHub in windows
    set DOCKER_REGISTRY_SERVER=docker.io
    set DOCKER_USER=your dockerID, same as for `docker login`
    set DOCKER_EMAIL=your dockerhub email, same as for `docker login`
    set DOCKER_PASSWORD=your dockerhub pwd, same as for `docker login`

    kubectl create secret docker-registry myregistrysecret --docker-server=$DOCKER_REGISTRY_SERVER --docker-username=$DOCKER_USER --docker-password=$DOCKER_PASSWORD --docker-email=$DOCKER_EMAIL
